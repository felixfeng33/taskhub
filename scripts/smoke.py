...
with tempfile.TemporaryDirectory(prefix="taskhub-smoke-") as directory:
    ...
    try:
        body = "# 跨设备任务\n\n- 保留正文和换行\n- Commit: example\n"
        created = json.loads(command(config_a, "add", "--title", "Shared task", "--body-file", "-", "--json", input=body))
        ...
        summaries = json.loads(command(config_a, "list", "--status", "review", "--json"))
        assert summaries == [{"id": created["id"], "title": "Shared task", "status": "review"}]
        process.send_signal(signal.SIGTERM)
        assert process.wait(timeout=15) == 0
        process = start()
        restored = json.loads(command(config_a, "show", task_id, "--json"))
        assert restored["body"] == body + "\nImplemented.\n" and restored["status"] == "review"
...
print("PASS: two clients, Markdown round-trip, claim conflict, server restart, acceptance")
